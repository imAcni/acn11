    ...
    <>
      <div>
        <div className="pb-6 pt-6">
          <h1 className="sm:hidden text-3xl font-extrabold leading-9 tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl sm:leading-10 md:text-6xl md:leading-14 uppercase">
            {title}
          </h1>
        </div>
          ...
          <div className="hidden max-h-screen h-full sm:flex flex-wrap bg-gray-50 dark:bg-gray-900/70 shadow-md pt-5 dark:shadow-gray-800/40 rounded min-w-[280px] max-w-[280px] overflow-auto">
            <div className="py-4 px-6">
              {pathname.startsWith('/posts') ? (
                <h3 className="text-primary-500 font-bold uppercase">all articles</h3>
              ) : (
                <Link
                  href={`/posts`}
          ...
          <div>
            <ul>
              {displayPosts.map((post) => {
                const { path, date, url, title, summary, tags } = post
                return (
                      ...
                      <div className="space-y-3">
                        <div>
                          <h2 className="text-2xl font-bold leading-8 tracking-tight">
                            <Link href={`/posts/${url}`} className="text-gray-900 dark:text-gray-100">
                              {title}
                            </Link>
                          </h2>
